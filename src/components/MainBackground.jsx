...
const MainBackground = () => {
  ...
  const [recommendFileText,setRecommendFileText]=useState("Upload your photo JPEG or PNG,max size: (500KB)")
  ...
  const handleFileUpload = (e) => {
    ...
    if (selectedFile) {
      ...
      if (selectedFile.size > 500 * 1024) {
        setOverLimit(true)
        setRecommendFileText("File is too Large please upload image under 500kb")
        return;
      }else{
         setOverLimit(false)
    ...
    >
      ...
      >
        <div className="button-to-upload-avatar">
          ...
          <img src={file || iconUpload} alt="iconUpload" className="uploaded-image" />
        </div>
        ...
        {file ? (
          <div className="button-group">
            <button className="transparent-button" onClick={() => setFile(null)}>
              Remove Image
            </button>
            <button
      ...
      />
        ...
        <div className='recomment-size-upload' style={{display:"flex",height:"15px"}}> 
        <img src={iconInfo} style={{marginRight:"10px",width:"12px"}} />
         <text style={{color: overLimit? "red":"white"}}>{recommendFileText}</text> 
        </div>
      ...
      <input className='input-full-name' value={fullName} onChange={(e)=>setFullName(e.target.value)}></input>
      </div>
      ...
      {invalidEmail && email !== "" && (
        <div className="recomment-size-upload" style={{ display: "flex", height: "15px" }}>
          <img src={iconInfo} style={{ marginRight: "10px", width: "12px" }} alt="info" />
      ...
      <div className='full-name'>
      <input className='input-full-name' value={githubUser} onChange={(e)=>setGithubUser(e.target.value)}></input>
      </div>
      </div>
